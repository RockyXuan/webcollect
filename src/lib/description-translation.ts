...
const DOMAIN_SUMMARIES: Array<{
  ...
  { domains: ["x.com", "twitter.com"], titleTerms: ["twitter", "x"], summary: "X/Twitter 社交平台，用于查看动态、关注话题和发布内容。" },
...
function summaryFromKnownSite(context: DescriptionContext, text: string): string | null {
  const hostname = hostnameFromUrl(context.url);
  const haystack = `${hostname} ${context.title || ""} ${text}`.toLowerCase();

  for (const item of DOMAIN_SUMMARIES) {
    if (item.domains.some((domain) => matchesDomain(hostname, domain))) return item.summary;
    if (item.titleTerms?.some((term) => haystack.includes(term.toLowerCase()))) return item.summary;
  }
  return null;
}
...
export function localizeDescriptionText(value?: string, context: DescriptionContext = {}): string {
  ...
  const known = summaryFromKnownSite(context, text);
