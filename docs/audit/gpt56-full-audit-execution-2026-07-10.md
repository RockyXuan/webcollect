...
- CI gate: pull requests and `main` now run the official production dependency audit, Vitest, the fail-fast legacy suite, the default Web build, extension build, built-artifact verification, and Playwright. The former ad-hoc shell loop was replaced by the same `test:legacy` command used locally.
